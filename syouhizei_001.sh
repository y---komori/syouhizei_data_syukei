#!/bin/bash
# syouhizei_001.sh  2019.1.28

###########################################################################
# このスクリプトについて
# 概要
# 消費税明細書データから、消費税計算で必要なデータを並べ替え及び集計する
...
# 使い方
# !!このディレクトリに移動後、右クリックで git bash起動後、 $ ./syouhizei_001.shを入力
...
| gawk 'BEGIN { FS=","; OFS=","; } NR >3 { if (match($22,"002")||match($22,"004")) print $0; }' \
...
paste -d , tmp4.csv tmp5.csv | cut -d "," -f 1,2,4,5,8,10,11,22 | nkf -s >> 科目別仮払消費税集計.csv
